...
# Эти ссылки открываются напрямую из меню (url-кнопки)
...
def sanitize_url_avito(url: str) -> Optional[str]:
    if not url:
        return None
    url = url.strip()
    if len(url) > 2000:
        return None
    if not (url.lower().startswith("https://avito.ru") or url.lower().startswith("https://www.avito.ru")):
        return None
    if re.search(r"[;'\\]", url):
        return None
    return url
...
def safe_url_display(url: str) -> str:
    if not url:
        return "— не задан —"
    return (url
        .replace("https://avito.ru", "https://av\u200bito.ru")
        .replace("https://www.avito.ru", "https://www.av\u200bito.ru")
    )
...
def resolve_config() -> Config:
    p = argparse.ArgumentParser(description="Единый бот (меню + парсер)")
    ...
    if not token or not db or days is None:
        raise SystemExit("Первый запуск: --token --db --days [--proxy --interval --max-items --profile-path]. Далее можно без аргументов (bot.config.json).")
...
class Storage:
    ...
    def set_watch_url(self, url: str) -> tuple[bool, str]:
        s = sanitize_url_avito(url)
        if not s: return False, "Недопустимый URL (только https://avito.ru/ ... )"
...
BTN_TOGGLE     = "toggle"
BTN_SET_URL    = "set_url"
# BTN_HELP и BTN_SUPPORT больше не используются как callback'и, т.к. теперь это url-кнопки

def build_menu_kb(running: bool) -> InlineKeyboardMarkup:
    toggle_text = "⏹ Стоп" if running else "▶️ Старт"

    # Кнопки помощи теперь сразу открывают URL (без callback_data)
...
def collect_ads(url: str, max_items: int, profile_path: str, proxy: Optional[str]) -> List[Dict]:
    ...
    try:
        driver.get(url)
        time.sleep(5)
        items = driver.find_elements(By.CSS_SELECTOR, '[data-marker="item-title"]')[:max_items]
        ...
        for item in items:
            ...
            try:
                parent = item.find_element(By.XPATH, "./ancestor::div[contains(@class,'iva-item-root')]")
                price = parent.find_element(By.CSS_SELECTOR, '[data-marker="item-price"]').text
            except:
                price = "цена не найдена"

            try:
                city = parent.find_element(By.CSS_SELECTOR, '.geo-root-BBVai span').text.strip()
            except:
                city = "город не найден"

            img = ""
            try:
                img_el = parent.find_element(By.CSS_SELECTOR, "[data-marker='item-photo'] img")
                img = img_el.get_attribute("src") or ""
            except:
                pass

            ads.append({"id": ad_id, "title": title, "href": href,
...
class ParserService:
    ...
    async def _run(self):
        ...
        if chat_id:
            try:
                await self.bot.send_message(chat_id=chat_id, text="Парсер запущен ✅")
            except Exception as e:
        ...
        try:
            while not stop_event.is_set():
                ...
                # сбор карточек (блокирующий selenium — но это допустимо в данном цикле)
...
async def menu_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == BTN_SET_URL:
        if locked or (expiry and now >= expiry):
            ...
            await q.edit_message_text("Пришли один URL (только https://avito.ru/...):",
                                      disable_web_page_preview=True)
        return

    # Кнопок с callback для помощи/поддержки больше нет — это url-кнопки.
...
async def main():
    ...
    app.bot_data["parser"] = parser_service
    ...
    asyncio.create_task(expiry_watchdog(app))
    try:
        await app.updater.start_polling(allowed_updates=Update.ALL_TYPES)
        await stop_event.wait()
    finally:
        await app.updater.stop()
        if parser_service.is_running():
